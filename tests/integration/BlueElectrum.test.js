...
describe('NomadElectrum', () => {
  it('ElectrumClient can estimate fees from histogram', async () => {
    assert.strictEqual(
      NomadElectrum.calcEstimateFeeFromFeeHistorgam(1, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      NomadElectrum.calcEstimateFeeFromFeeHistorgam(18, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      NomadElectrum.calcEstimateFeeFromFeeHistorgam(144, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
  ...
  it('ElectrumClient can test connection', async () => {
    ...
    assert.ok(!(await NomadElectrum.testConnection('google.com', false, 443)));
    assert.ok(!(await NomadElectrum.testConnection('google.com', 443, false)));
    assert.ok(!(await NomadElectrum.testConnection('joyreactor.cc', false, 443)));
    assert.ok(!(await NomadElectrum.testConnection('joyreactor.cc', 443, false)));
  ...
  it('ElectrumClient can estimate fees', async () => {
    assert.ok((await NomadElectrum.estimateFee(1)) > 1);
  ...
  it('NomadElectrum can do getBalanceByAddress()', async function () {
    const address = '3GCvDBAktgQQtsbN6x5DYiQCMmgZ9Yk8BK';
    const balance = await NomadElectrum.getBalanceByAddress(address);
  ...
  it('NomadElectrum can do getTransactionsByAddress()', async function () {
    const txs = await NomadElectrum.getTransactionsByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
  ...
  it.skip('NomadElectrum can do getMempoolTransactionsByAddress()', async function () {
    const txs = await NomadElectrum.getMempoolTransactionsByAddress('bc1qp33en9mnw277c9vz5fz9vcu666cvervdnk02327wwph97hdjurqqxtl03c');
    assert.ok(txs.length > 0);
    assert.ok(txs[0].tx_hash);
    assert.ok(txs[0].fee);

    const rez = await NomadElectrum.multiGetTransactionByTxid([txs[0].tx_hash], 10, true);
    assert.ok(rez[txs[0].tx_hash]);
  });

  it('NomadElectrum can do getTransactionsFullByAddress()', async function () {
    const txs = await NomadElectrum.getTransactionsFullByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
    for (const tx of txs) {
  ...
  it('NomadElectrum can do txhexToElectrumTransaction()', () => {
    const tx =
    ...
    const decoded = NomadElectrum.txhexToElectrumTransaction(tx);
  ...
  it.each([false, true])('NomadElectrum can do multiGetBalanceByAddress(), disableBatching=%p', async function (diableBatching) {
    if (diableBatching) NomadElectrum.setBatchingDisabled();
    const balances = await NomadElectrum.multiGetBalanceByAddress([
    ...
    if (diableBatching) NomadElectrum.setBatchingEnabled();
  });

  it('NomadElectrum can do multiGetUtxoByAddress()', async () => {
    const utxos = await NomadElectrum.multiGetUtxoByAddress(
      [
        'bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh',
        'bc1qvd6w54sydc08z3802svkxr7297ez7cusd6266p',
        'bc1qwp58x4c9e5cplsnw5096qzdkae036ug7a34x3r',
  ...
  it.each([false, true])('ElectrumClient can do multiGetHistoryByAddress(), disableBatching=%p', async disableBatching => {
    if (disableBatching) NomadElectrum.setBatchingDisabled();
    const histories = await NomadElectrum.multiGetHistoryByAddress(
      [
        'bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh',
        'bc1qvd6w54sydc08z3802svkxr7297ez7cusd6266p',
        'bc1qwp58x4c9e5cplsnw5096qzdkae036ug7a34x3r',
    ...
    if (disableBatching) NomadElectrum.setBatchingEnabled();
  });

  it.each([false, true])('ElectrumClient can do multiGetTransactionByTxid(), disableBatching=%p', async disableBatching => {
    if (disableBatching) NomadElectrum.setBatchingDisabled();
    const txdatas = await NomadElectrum.multiGetTransactionByTxid(
      [
        'ad00a92409d8982a1d7f877056dbed0c4337d2ebab70b30463e2802279fb936d',
