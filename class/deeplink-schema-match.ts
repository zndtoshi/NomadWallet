...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString: string): boolean {
    ...
    return (
      ...
      lowercaseString.startsWith('nomadwallet:') ||
  ...
  static navigationRouteFor(
    ...
    if (event.url.toLowerCase().startsWith('nomadwallet:bitcoin:') || event.url.toLowerCase().startsWith('nomadwallet:lightning:')) {
      event.url = event.url.substring(11);
    } else if (event.url.toLocaleLowerCase().startsWith('nomadwallet://widget?action=')) {
      event.url = event.url.substring('nomadwallet://'.length);
    }
    ...
    if (isBothBitcoinAndLightning) {
      ...
      (async () => {
        if (urlObject.protocol === 'nomadwallet:' || urlObject.protocol === 'lapp:' || urlObject.protocol === 'blue:') {
  ...
  /**
   * Extracts server from a deeplink like `nomadwallet:setelectrumserver?server=electrum1.nomadwallet.io%3A443%3As`
  ...
  static getServerFromSetElectrumServerAction(url: string): string | false {
    if (!url.startsWith('nomadwallet:setelectrumserver') && !url.startsWith('setelectrumserver')) return false;
  ...
  /**
   * Extracts url from a deeplink like `nomadwallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com`
   * returns FALSE if none found
   *
   * @param url {string}
   * @return {string|boolean}
  ...
  static getUrlFromSetLndhubUrlAction(url: string): string | false {
    if (!url.startsWith('nomadwallet:setlndhuburl') && !url.startsWith('setlndhuburl')) return false;
